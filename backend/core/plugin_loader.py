...
class FunctionToolWrapper:
    ...
    ... (docstring and __init__ are unchanged) ...
    """
    ...
    def _generate_openai_schema(self) -> Dict[str, Any]:
        ...
        # Filter out special parameters from OpenAI schema
        special_params = {"thread_id", "user_id", "turn_correlation_id"}
        properties = json_schema.get("properties", {})
        required = json_schema.get("required", [])

        # Remove special parameters from properties and required list
        filtered_properties = {k: v for k, v in properties.items() if k not in special_params}
        filtered_required = [r for r in required if r not in special_params]
    ...
    async def execute(self,
        ...
        This method is a "delegating generator." It consumes the
        wrapped function (which is a coroutine or generator)
        and re-yields its progress, then returns its final value.

        This ensures that `execute()` *always* returns an async generator,
        ...
        try:
            ...
            # Validation failed. We must still be a generator.
            # Yield an error and return.
            logger.error(f"Tool argument validation failed for {self.func_name}: {e}")
            yield f"Error: Invalid arguments for {self.func_name}: {str(e)}"
            return  # This raises StopAsyncIteration

        # 2. Add special parameters to args_dict if they were in the original signature
        # Note: args_dict contains ALL parameters from the Pydantic model, including
        # those not in the incoming arguments (they'll be None or have default values)
        special_params = {
            "user_id": user_id,
            "thread_id": thread_id,
        ...
        for param_name, param_value in special_params.items():
            if param_name in args_dict:
                # The parameter was in the signature, so we add the provided value
                args_dict[param_name] = param_value

        # 2. Get the actual tool callable (coroutine or generator)
        if self.is_method:
            ...
            gen_or_coro = method(**args_dict)

        else:
            # Plain function
            gen_or_coro = self.func(**args_dict)

        # 3. Wrap the tool's result (Wrap 1)
        wrapped = GeneratorWrapper(gen_or_coro)

        # 4. Delegate: Yield its yields... (exceptions get passed upstream)
        async for item in wrapped.yields():
            yield item  # This line makes execute() a generator

        # 5. Delegate: Get its return and return it
...
class PluginLoader:
    ...
    def inject_manager(self, manager: "PluginManager") -> None:
        """Called once by PluginManager after both objects exist."""
    ...
    def _load_function_plugins(self):
        ...
        This method discovers and loads function plugins from the plugins/functions/ directory.
        Function plugins are classes that implement the FunctionPlugin protocol.
        The loading process works as follows:
        1. Scan plugins/functions/ directory for .py files (excluding __init__.py)
        2. For each file, dynamically import it as a Python module
        3. Inspect the module for classes that implement the FunctionPlugin interface
        4. Instantiate each plugin class and add it to the function_plugins list
        5. Sort plugins by priority (highest first)
        """
        functions_dir = self.plugins_dir / "functions"
        if not functions_dir.exists():
            logger.debug("Function plugins directory not found - no function plugins to load")
            return

        logger.info(f"Loading function plugins from: {functions_dir}")

        # Track if we found any valid plugins in each module
        modules_with_valid_plugins = []
        modules_without_valid_plugins = []
        ...
        for plugin_file in functions_dir.glob("*.py"):
            ...
            logger.debug(f"Processing function plugin file: {plugin_file}")
            ...
            # inject the plugin manager, in case the module needs to use core
            module.__dict__["plugin_manager"] = self._manager
            ...
            try:
                ...
                logger.error(f"Failed to import module {module_name}: {e}", exc_info=True)
                modules_without_valid_plugins.append(plugin_file.name)
                continue

            # Find function plugin classes using duck typing
            found_valid_plugin = False
            for name, obj in inspect.getmembers(module, inspect.isclass):
                # Skip private classes and classes imported from other modules
                if name.startswith("_") or obj.__module__ != module.__name__:
                    continue
                ...
                # Check if class implements FunctionPlugin interface using duck typing
                try:
                    # Try to instantiate and check if it implements FunctionPlugin
                    plugin_instance = obj()
                    if isinstance(plugin_instance, FunctionPlugin):
                        # Successfully found a valid function plugin
                        self.function_plugins.append(plugin_instance)
                        found_valid_plugin = True
                        logger.info(f"Loaded function plugin '{name}' (priority: {plugin_instance.get_priority()}, file: {plugin_file.name})")
                    else:
                        logger.debug(f"Class '{name}' does not implement FunctionPlugin protocol - skipping")
                except Exception as e:
                    logger.error(f"Failed to instantiate or validate function plugin class '{name}': {e}", exc_info=True)

            if found_valid_plugin:
                modules_with_valid_plugins.append(plugin_file.name)
            else:
                logger.warning(f"No valid function plugin classes found in module: {plugin_file.name}")
                modules_without_valid_plugins.append(plugin_file.name)

        # First, group plugins by name to check for duplicates
        name_groups = {}
        for plugin in self.function_plugins:
            name = plugin.get_name()
            if name not in name_groups:
                name_groups[name] = []
            name_groups[name].append(plugin)

        # Handle duplicate names first
        deduplicated_plugins = []
        for name, duplicate_plugins in name_groups.items():
            if len(duplicate_plugins) > 1:
                logger.error(f"DUPLICATE FUNCTION PLUGIN NAME: Found {len(duplicate_plugins)} plugins with name '{name}'. Keeping only one.")
                for i, dup_plugin in enumerate(duplicate_plugins):
                    logger.error(f"  - Duplicate {i+1}: {dup_plugin.__class__.__name__} from module {dup_plugin.__class__.__module__}")
                # Keep only the first one
                deduplicated_plugins.append(duplicate_plugins[0])
            else:
                deduplicated_plugins.extend(duplicate_plugins)

        # Now check for priority ties among the deduplicated plugins
        priority_groups = {}
        for plugin in deduplicated_plugins:
        ...
        # Log priority ties
        for priority, plugins in priority_groups.items():
            if len(plugins) > 1:
                sorted_names = sorted(p.get_name() for p in plugins)
                logger.error(f"PRIORITY TIE: {len(plugins)} function plugins share priority {priority}: {sorted_names}")

        # Final sort: by priority (descending), then by name (ascending)
        self.function_plugins = deduplicated_plugins
        self.function_plugins.sort(key=lambda p: (-p.get_priority(), p.get_name()))

        # Log summary
        logger.info(f"Function plugin loading complete. Loaded {len(self.function_plugins)} function plugins from {len(modules_with_valid_plugins)} modules")
        
        if modules_without_valid_plugins:
            logger.warning(f"Modules without valid function plugins: {modules_without_valid_plugins}")

        # Log the final priority order for debugging
        if self.function_plugins:
            plugin_names = [f"{p.get_name()}({p.get_priority()})" for p in self.function_plugins]
            logger.debug(f"Function plugins (sorted by priority, then name): {plugin_names}")
